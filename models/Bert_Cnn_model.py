...
def prepare_data(file_path, tokenizer):
    ...
    # Use all CPU cores for DataLoader
    num_workers = 48  # Adjust based on your system's CPU count
...
def train_and_evaluate(model, train_loader, test_loader, num_epochs, device, label_encoder):
    ...
    # Map predicted labels back to their class names using LabelEncoder
    predicted_labels = label_encoder.inverse_transform(predictions)
    true_labels_mapped = label_encoder.inverse_transform(true_labels)

    # Generate classification report and confusion matrix
    logging.info("\n" + classification_report(true_labels_mapped, predicted_labels, target_names=label_encoder.classes_))
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig('bert_cnn_training_validation_accuracy.png', dpi=500)
    plt.show()
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('bert_cnn_training_validation_loss.png', dpi=500)
    plt.show()
...
if __name__ == "__main__":
    ...
    # Load pretrained BERT from your directory
    ...
    # Prepare data
    train_loader, test_loader, label_encoder = prepare_data('../data/mxm_msd_genre.cls', tokenizer)
