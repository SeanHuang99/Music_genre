...
# Define a more complex FCNN model from the second code
...
def main_worker(rank, world_size):
    ...
    # Assuming each word and count combination can be represented as a vector of size 128
    input_dim = 128  # 你可能需要调整这个值
    output_dim = len(df['genre'].unique())

    # Initialize FCNN (replacing SimpleFCNN)
    model = FCNN(input_dim, output_dim)

    logger.info(f"Initialized FCNN model with input dimension {input_dim} and output dimension {output_dim}")

    model.to(rank)
    model = DDP(model, device_ids=[rank])

    def encode_texts(texts):
        # Here you need a proper embedding conversion
        # For simplicity, let's assume you have a precomputed embedding for each word+count combination
        # Replace this with actual text embedding logic
        return torch.rand(len(texts), input_dim)  # Random embeddings for example purposes

    X_train_embeddings = encode_texts(train_data['word'] + ' ' + train_data['count'].astype(str))
    logger.debug(f"Encoded {len(train_data)} training texts into embeddings")

    X_test_embeddings = encode_texts(test_data['word'] + ' ' + test_data['count'].astype(str))
    logger.debug(f"Encoded {len(test_data)} test texts into embeddings")
    ...
    for epoch in range(start_epoch, 5):
        ...
        for batch_idx, (b_embeddings, b_labels) in enumerate(train_loader):
            b_embeddings = b_embeddings.to(rank)
            b_labels = b_labels.to(rank)
        ...
        with torch.no_grad():
            for b_embeddings, b_labels in test_loader:
                b_embeddings = b_embeddings.to(rank)
                b_labels = b_labels.to(rank)
    ...
    if rank == 0:
        ...
        # Save the model and state dictionaries
        torch.save(model.state_dict(), './pretrained_fcnn/fcnn_model.pt')
        torch.save(optimizer.state_dict(), './pretrained_fcnn/optimizer.pt')
        torch.save(scaler.state_dict(), './pretrained_fcnn/scaler.pt')
        logger.info(f"Model and state dictionaries saved to './pretrained_fcnn'")
