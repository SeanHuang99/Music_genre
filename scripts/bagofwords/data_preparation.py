...
def preprocess_words(df):
    ...
    for index, row in df.iterrows():
        ...
        counts_list.append(' '.join(counts))
...
def prepare_dataset():
    ...
    # 并行处理，不去除停用词，保留 'trackId' 和 'genre'
    ddf = ddf.map_partitions(lambda df: preprocess_words(df), meta=meta)
    ...
    # 保存处理后的数据
    final_df.to_csv('../../data/mxm_msd_genre_pro_no_stopwords.cls', index=False)
    print("New dataset created and saved as 'mxm_msd_genre_pro_no_stopwords.cls'")
